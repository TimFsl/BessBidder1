"""
Train PPO Agent — v12: same as v10 (precomputed RI lookup, obs/reward shape) but
intraday (IDC) reward is injected without a complete-cycle gate — only
``START_IDC_STEPS`` applies (open positions).
"""
...
from src.coordinated_multi_market.v12_baseline_open_positions.basic_battery_dam_env import (
    BasicBatteryDAM,
)
from src.coordinated_multi_market.v12_baseline_open_positions.custom_ppo import (
    CustomPPO,
)
from src.coordinated_multi_market.v12_baseline_open_positions.learning_utils import (
    load_input_data,
    prepare_input_data,
    linear_schedule,
)
...
if __name__ == "__main__":
    ...
    if RESUME_TRAINING:
        ...
        print("Starting training from scratch (v12 open positions, precomputed RI lookup).")
        model = CustomPPO("MlpPolicy", env, **model_kwargs)
