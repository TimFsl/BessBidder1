...
def solve_bucket_with_persistent_model(
...
) -> Tuple[Optional[pd.DataFrame], pd.DataFrame, float, float]:
    """
    ...
    return results, trades, m.ObjVal, r_long_remaining_mwh_new
...
def simulate_days_stacked_quarterhourly_products(
    ...
    (
        ...
        da_long_budget_constr,
        free_sell_budget_constr,
    ) = build_battery_model(T, cap=1, c_rate=c_rate, roundtrip_eff=roundtrip_eff)
    ...
    # Initialize DA-long state if DRL DA trades exist
    if day_ahead_trades_drl is not None and not day_ahead_trades_drl.empty:
        try:
            da_buys = day_ahead_trades_drl[
                day_ahead_trades_drl["side"] == "buy"
            ]
            if len(da_buys) > 0:
            ...
            r_long_remaining_mwh = max(
                0.0, buy_mwh * (roundtrip_eff ** 0.5) - sell_mwh / (roundtrip_eff ** 0.5)
            )
