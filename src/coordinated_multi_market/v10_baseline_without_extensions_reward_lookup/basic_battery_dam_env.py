...
class BasicBatteryDAM(gym.Env):
    ...
    def step(
        ...
        if realized_quantity < 0.0:
            # Laden
            # v10 variant: apply RTE only on the sell-side cashflow.
            profit = -clearing_price * (-realized_quantity)
            reward = -clearing_price * (-realized_quantity) / (10 * float(self._capacity))
        elif realized_quantity > 0.0:
            # Entladen
            # Original behavior (active): apply sqrt(RTE) on sell side.
            profit = clearing_price * (realized_quantity * self._efficiency)
            reward = clearing_price * (realized_quantity) / (10 * float(self._capacity))
            # Alternative (inactive): apply full RTE on sell side only.
            # profit = clearing_price * (realized_quantity * (self._efficiency**2))
            # reward = clearing_price * (realized_quantity * (self._efficiency**2)) / (10 * float(self._capacity))
        else:
            profit = 0.0
            reward = 0.0
