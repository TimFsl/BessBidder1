"""
Train PPO Agent — ``v13_baseline_counterfactual_reward``:
same stack as v10 (precomputed summary lookup, env, scaler) but PPO uses **counterfactual**
intraday shaping (replay + lookup margins) as in ``v3_counterfactual_reward_lookup``,
not the uniform per-step RI injection from v10.
"""
...
from src.coordinated_multi_market.v13_baseline_counterfactual_reward.custom_ppo import (
    CustomPPO,
)
...
if __name__ == "__main__":
    ...
    model_kwargs = dict(
        ...
        gamma=0.999,
        use_counterfactual_ri_reward=True,
        ...
        reward_normalization_scale_multiplier=1.0,
    )
    ...
    if RESUME_TRAINING:
        ...
        print("Starting training from scratch (v13 counterfactual RI, precomputed lookup).")
        model = CustomPPO("MlpPolicy", env, **model_kwargs)
