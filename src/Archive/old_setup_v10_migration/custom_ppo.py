...
class CustomPPO(PPO):
    ...
    def collect_rollouts(
            ...
            if run_ri:
                ...
                if self.intraday_product_type != "QH":
                    raise NotImplementedError(
                        "v10_baseline_without_extensions_reward_lookup only supports "
                ...
                # Keep day-ahead profit tracking on the direct environment-based
                # calculation. Only the intraday component comes from lookup.
        ...
        if ep_profit_combined:
            ...
            self.logger.record(
                "episode_profit/day_ahead_eur_plus_intraday_lookup",
                float(np.mean(ep_profit_combined)),
            )
            self.logger.record(
