...
def solve_bucket_with_persistent_model(
    ...
    - free_sell_budget_constr: energy budget constraint for free sells (RHS = free_mwh = cap - r_long)
    ...
    # NEW
    # free_mwh represents the portion of capacity not reserved for DA-long unwind
    cap = 1.0  # only temporary (replace with actual cap)
    free_mwh = max(0.0, float(cap) - float(r_long_remaining_mwh))
...
def simulate_days_stacked_quarterhourly_products(
    ...
    while current_day < end_day:
        ...
        # p_da_buy is None if there were no DA buys
        try:
